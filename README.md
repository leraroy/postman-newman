...
```
newman run HW5.postman_collection.json -e ClickUpHW.postman_environment.json
```
Newman also supports reporters.  
For instance, to use the Newman HTMLExtra Reporter go to the terminal execute  
**npm install -g newman-reporter-htmlextra**
```
newman run HW5.postman_collection.json -e ClickUpHW.postman_environment.json -r htmlextra
```
